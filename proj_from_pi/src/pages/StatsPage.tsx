...
export default function StatsPage() {
  ...
  const [lotId, setLotId] = useState<string>('times-mockup-01')
  const [lotOptions, setLotOptions] = useState<Array<{ id: string; name: string }>>([])

  // Fetch lot list for the selector
  useEffect(() => {
    fetch('/admin/lots')
      .then(r => r.json())
      .then((lots: Array<{ id: string; name: string }>) => {
        setLotOptions(lots.map(l => ({ id: l.id, name: l.name })))
        if (lots.length > 0 && !lots.find(l => l.id === lotId)) {
          setLotId(lots[0].id)
        }
      })
      .catch(() => {})
  ...
  useEffect(() => {
    const fetchAll = () => {
      fetch(`/api/v1/lots/${lotId}/history?limit=200`)
        .then(r => r.json())
        .then(setHistory)
        .catch(() => {})
      ...
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        ...
        {lotOptions.length > 1 && (
          <div style={{ marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11, color: COLORS.muted, fontWeight: 600 }}>LOT:</span>
              ...
              {lotOptions.map(l => (
                <button
                  ...
                  style={{
                    ...
                    background: lotId === l.id ? '#252a3e' : 'transparent',
                    border: lotId === l.id ? '1px solid #F7C12E44' : '1px solid #252a3e',
                    color: lotId === l.id ? '#F7C12E' : '#6b7280',
