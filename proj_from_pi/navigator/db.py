...
def init_db():
    ...
    conn.executescript("""
        ...
        CREATE INDEX IF NOT EXISTS idx_ingest_lot_ts
            ON ingest_log (lot_id, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_space_events_lot_ts
            ON space_events (lot_id, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_space_events_space
            ON space_events (space_id, timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_error_log_ts
            ON error_log (timestamp DESC);
        CREATE INDEX IF NOT EXISTS idx_error_log_component
            ON error_log (component, timestamp DESC);
...
def log_error(component: str, error_type: str, message: str, detail: str = None):
    try:
        ...
        log.error(f"error_log write failed: {e}")
...
def cleanup_old_data():
    ...
    conn = get_db()
    for table in ("ingest_log", "space_events", "error_log"):
        cur = conn.execute(f"DELETE FROM {table} WHERE timestamp < ?", (cutoff,))
        if cur.rowcount > 0:
            log.info(f"Retention: deleted {cur.rowcount} rows from {table} (older than {RETENTION_DAYS}d)")
    conn.commit()
    conn.close()
